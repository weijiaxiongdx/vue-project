...
<script lang="ts" setup> // lang设置为ts，但script中也可以写js，即ts和js都可以写
    ...
    //1.props方式(使用频率最高)：父传子，子标签上直接写；子传父，子组件中调用父组件传递给子组件的函数
    //2.自定义事件方式：用于子传父
    //3.mitt方式：实现任意组件之间的通信(一方绑定事件、另一方触发事件)。安装npm i mitt、创建emitter.ts文件、main.js中导入
    //4.v-model方式：
    //5.$attrs：实现当前组件的父组件向当前组件的子组件传递数据，祖传孙，也可以实现孙传祖。$attrs是一个对象，包含所有父组件传入的标签属性(只包含没有使用defineProps接收的属性，接收过的在props对象中)
